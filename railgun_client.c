...
int main(int argc, char** argv) {
	...
	void *data_buffer;
	int i = 0, nfds = 0, nbytes = 0, data_read_size = 0;
	int data_fd = 0;
	long filelength = 0;
	...
	while (1) {
		...
		for (i = 0; i < nfds; i++) {
			if (events[i].events & EPOLLIN) {
				...
				nbytes = read(g_sock_fd, recvbuf, MAXBUF);
				if (nbytes < 0) {
					if (errno != EAGAIN) {
						perror("read error");
					}
					...
					RAILGUN_HEADER* packet = payload_queue_find(ack);
					if (packet != NULL) {
				...
				for_packet_in_payload_queue(packet, tmp)
				{
					if (packet->timestamp + (u_int64_t)RTO <= current_time_in_millis) {
						railgun_packet_write(packet, g_sock_fd, sendbuf, data_buffer, NULL);
				...
				if (!is_packet_send) {
					if (data_read_size + PAYLOAD_SIZE <= filelength) {
						...
						RAILGUN_HEADER* pay_load = payload_queue_add(g_seq, g_ack, data_read_size,
								get_current_time_in_millis());
						printf("add %d to table \n", seq);
						railgun_packet_write(pay_load, g_sock_fd, sendbuf, data_buffer, &packet_skip_size);
						...
						if (is_payload_queue_empty) {
							...
							ev.events = EPOLLIN;
							int ret = epoll_ctl(g_kdp_fd, EPOLL_CTL_MOD, g_sock_fd,
									&ev);
							if (ret < 0) {
